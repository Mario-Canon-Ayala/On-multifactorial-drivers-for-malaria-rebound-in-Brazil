########################################################################
#                                                                      #
#    Bayesian models and falciparum incidence prediction (AMAPÁ)       #
#                                                                      #
########################################################################
### Packages
library(foreign)
...
formula.4<- Y2 ~ 1 +  f(month, model = "rw2", constr = T, cyclic = T) + f(year, model = "iid", constr = T) +  f(ID.area.sta, model = "iid") +f(ID.area.sta2,model="bym",graph=AP.adj) + f(i,model="rw1")
t <- proc.time()
