...
MA.adj <- paste(getwd(),"/MA.graph",sep="")
############# Order data ############
BASE_STATE<-NULL
for (j in 1:192) {
  BASEORD<-subset(BASE_BRA_STA,BASE_BRA_STA$i==j)
  order<-match(MAR@data[["CD_GEOCMU"]],BASEORD$muni_cut.CD_GEOCMU.j.)
...
formula.4<- Y2 ~ 1 +  f(month, model = "rw2", constr = T, cyclic = T) + f(year, model = "iid", constr = T) +  f(ID.area.sta, model = "iid") +f(ID.area.sta2,model="bym",graph=MA.adj) + f(i,model="rw1")
t <- proc.time()
...
proc.time()-t
# SAVE RESULTS
#save(model.inla.5,file="MA_after_INLA_mes_viv_Ypred2016_2018_simples+spa_month_year_ate_2018.rdata")
load(file="MA_after_INLA_mes_viv_Ypred2016_2018_simples+spa_month_year_ate_2018.rdata")
...
save(Inci,file="MA_pre_viv.rdata")
...
# SAVE RESULTS
# save(BASSER,file="MAPER_MA_viv.rdata")
