...
setwd("~/Bayesian models and incidence prediction/PA")
...
formula.1<- Y ~ 1 + f(ID.area.sta,model="bym",graph=PA.adj) + f(i,model="rw1") + f(ID.mun.ano, model="iid") + despues*AGRECU + despues*TUR_V + despues*Desma  + despues*DOM  + despues*GAR_MIN + despues*OUT +despues*IMPO
...
#PA_viv <- ((exp(model.inla.1$summary.fixed*0.01) - 1)*100)[10:16,] 
PA_viv<-model.inla.1$summary.fixed[10:16,] 
#save(PA_viv,file="PA_viv_variavels.Rdata")
...
formula.1<- Y ~ 1 + f(ID.area.sta,model="bym",graph=PA.adj) + f(i,model="rw1") + f(ID.mun.ano, model="iid") + despues*Desma  + despues*AGRECU + despues*TUR_V  + despues*DOM  + despues*GAR_MIN + despues*OUT +despues*IMPO
######## Run code to make INLA to 2018
...
## PARA falciparum results
model.inla.1$summary.fixed
model.inla.1$dic$dic
# incidence increase from 1% of variable increase
PA_fal<-model.inla.1$summary.fixed[10:16,] 
#save(PA_fal,file="PA_fal_variavels.Rdata")
