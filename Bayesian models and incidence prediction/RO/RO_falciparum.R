########################################################################
#                                                                      #
#    Bayesian models and falciparum incidence prediction (RONDONIA)    #
#                                                                      #
########################################################################
### Packages
library(foreign)
...
formula.4<- Y2 ~ 1 +  f(month, model = "rw2", constr = T, cyclic = T) + f(year, model = "iid", constr = T) +  f(ID.area.sta, model = "iid") +f(ID.area.sta2,model="bym",graph=RO.adj) + f(i,model="rw1")
t <- proc.time()
...
proc.time()-t
# save results
#save(model.inla.5,file="RO_after_INLA_mes_fal_Ypred2016_2018_simples+spa_month_year_ate_2018.rdata")
load(file="RO_after_INLA_mes_fal_Ypred2016_2018_simples+spa_month_year_ate_2018.rdata")
...
# save results
# save(Inci,file="RO_pre_fal.rdata")
...
# save results
# save(BASSER,file="MAPER_RO_fal.rdata")